# vm_translator/vm_code_emitter.py
...
class VMCodeEmitter:
    ...
    def write_call(self, function, num_arguments):
        ...
        instructions = [f'\t@{return_address}', '\tD=A', '\t@SP', '\tM=M+1', '\tA=M-1', '\tM=D', f'\t//Function Call: Save Caller State', self._push_segment('LCL'), self._push_segment('ARG'), self._push_segment('THIS'), self._push_segment('THAT'), f"\t//Function Call: Reposition ARG and LCL, then call function", f'\t@{5 + num_arguments}', '\tD=A', '\t@SP', '\tD=M-D\t// ARG = SP - 5 - n_args', '\t@ARG', '\tM=D\t\t// Reposition ARG', '\t@SP', '\tD=M', '\t@LCL', '\tM=D\t\t// Reposition LCL', f'\t@{function}\t//Jump to function {function}', '\t0;JMP', f'({return_address})']
        self._write_instructions(instructions)

    @staticmethod
    def _push_segment(segment):
        instructions = [f'\t@{segment}', '\tD=M', '\t@SP', '\tM=M+1\t// Increment SP', '\tA=M-1', f'\tM=D\t\t// Push {segment}']
        return '\n'.join(instructions)

    def write_return(self):
        self._write_comment('return')
        instructions = ['\t@LCL', '\tD=M', '\t@R13', '\tM=D', '\t@5', '\tA=D-A', '\tD=M', '\t@R14', '\tM=D', '\t@SP', '\tA=M-1', '\tD=M', '\t@ARG', '\tA=M', '\tM=D', '\tD=A+1', '\t@SP', '\tM=D', '\t@R13', '\tAM=M-1', '\tD=M', '\t@THAT', '\tM=D', '\t@R13', '\tAM=M-1', '\tD=M', '\t@THIS', '\tM=D', '\t@R13', '\tAM=M-1', '\tD=M', '\t@ARG', '\tM=D', '\t@R13', '\tAM=M-1', '\tD=M', '\t@LCL', '\tM=D', '\t@R14', '\tA=M', '\t0;JMP']
        self._write_instructions(instructions)
    ...
    def _generate_arithmetic_instructions(command, unique_num):
        ...
        equality_operation = ['\tD=M-D', '\t@{op}_{unique}', '\tD;J{op}', '\tD=0', '\t@FINALIZE_{unique}', '\t0;JMP', '({op}_{unique})', '\tD=-1', '(FINALIZE_{unique})', '\t@SP', '\tA=M-1']
        instruction = []
    ...
    def _generate_push_pop_snippets(segment, index):
        seg_to_d = [f'\t@{index}', '\tD=A', '\t@' + ('5' if segment == 'temp' else '{seg}'), '\tA=D+' + ('A' if segment == 'temp' else 'M'), '\tD=M']
        d_to_stack = ['\t@SP', '\tM=M+1', '\tA=M-1', '\tM=D']
        stack_to_d = ['\t@SP', '\tM=M-1', '\tA=M', '\tD=M']
        d_to_seg = ['\t@R13', '\tM=D', f'\t@{index}', '\tD=A', '\t@' + ('5' if segment == 'temp' else '{seg}'), '\tD=D+' + ('A' if segment == 'temp' else 'M'), '\t@R14', '\tM=D', '\t@R13', '\tD=M', '\t@R14', '\tA=M', '\tM=D']
